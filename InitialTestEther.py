...
TCP_Port = 5025
>>>>>>> a944af7c71160bbbe943665be2fac5546e4ad5f3
...
send(s,'FORM:DATA ASCII')

send(s,'OUTP:SMOD HIMP')  # turn on high-impedance output mode
send(s,'SENS:CURR:RSEN ON')  # set to 4-wire sense mode  # OFF = 2-Wire mode
send(s,'SENS:FUNC "CURR"')  # set measure, sense, to current
send(s,f'SENS:CURR:RANG {currentRange}')  # set current range # can also be 'SENS:CURR:RANG:AUTO ON'
send(s,'SENS:CURR:UNIT AMP')  # set measure units to Ohm, can also be Watt or Amp
send(s,'SOUR:FUNC VOLT')  # set source to voltage
send(s,f'SOUR:VOLT {sourceVoltage}')  # set output voltage => discharge or charge test
send(s,'SOUR:VOLT:READ:BACK ON')  # turn on source read back
send(s,f'SOUR:VOLT:RANG {voltageRange}')  # set source range
send(s,f'SOUR:VOLT:ILIM {sourceLimit}')  # set source (current) limit
send(s,'OUTP ON')  # turn on output, source
...
for i in range(10):
<<<<<<< HEAD
    send('READ? "defbuffer1", SOUR')  # a ? is used for a query command otherwise is a set command
    voltage_i = float(recieve())  # defbuffer1 returns sense value
...
=======
    send(s,'READ? "defbuffer1", SOUR')  # a ? is used for a query command otherwise is a set command
    voltage_i = recieve(s)  # defbuffer1 returns sense value
    vocL.append(voltage_i)
    print(voltage_i)
    time.sleep(0.1)

send(s,'OUTP OFF')  # turn off keithley
>>>>>>> a944af7c71160bbbe943665be2fac5546e4ad5f3
...
for i in range(len(sourceLimit)):
    ...
    curr = recieve()  # units = Ohm
    currentL_impedance.append(float(curr))

    send('READ? "defbuffer1", SOUR')  # a ? is used for a query command otherwise is a set command
    volt = float(recieve())  # defbuffer1 returns sense value
